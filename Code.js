...
function promptUserForBulletPoint() {
  var ui = DocumentApp.getUi()
  var response = ui.alert('Add a bullet point annotation to link?', ui.ButtonSet.YES_NO);
  var result = ''
...
var parent = paragraph.getParent()
  ...
  return [
    ...
    parent.insertParagraph(i + 1, ghLineUrl).setLinkUrl(ghLineUrl),
    parent.insertParagraph(i + 2, ''),
    parent.insertParagraph(i + 3, Utilities.formatString('"%s" should link to %s.', linkText, targetPath.replace(/\\/ig, '/'))),
    parent.insertParagraph(i + 4, '')
  ]
}
...
function addIssueHeadings() {
  ...
  var startAt = parseInt(DocumentApp.getUi().prompt('Enter issue number to start at:').getResponseText())
